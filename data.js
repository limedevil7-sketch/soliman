...
const VIDEOS = [
  {
    ...
    youtubeUrl: 'https://www.youtube.com/watch?v=SaukCBkCkqU',
  ...
  },
  {
    ...
    youtubeUrl: 'https://www.youtube.com/watch?v=3sNfGOSQxwo',
    category: 'MYSTERY',
    duration: '14:20',
    date: '2026-08-10',
    featured: false
  },
  {
    ...
    youtubeUrl: 'https://www.youtube.com/watch?v=iHJavBQP9u8',
    category: 'AI HORROR',
    duration: '22:05',
    date: '2026-08-05',
    featured: false
  },
  {
    ...
    youtubeUrl: 'https://www.youtube.com/watch?v=CO-Zeojk63w',
    category: 'DARK WEB',
    duration: '19:33',
    date: '2026-07-28',
    featured: false
  },
  {
    ...
    youtubeUrl: 'https://www.youtube.com/watch?v=iYGZUZJM6JE',
    category: 'HORROR',
    duration: '16:48',
    date: '2026-07-20',
    featured: false
  },
  {
    ...
    youtubeUrl: 'https://www.youtube.com/watch?v=dQ6GzuAMjM0',
...
const MessagesStore = {
  STORAGE_KEY:'sn_messages_v2', MAX_LENGTH:280, RATE_LIMIT_MS:8000, lastSubmit:0, backend:null,
  init(){try{const c=window.SN_SUPABASE_CONFIG||{};if(c.url&&c.anonKey&&window.supabase)this.backend=window.supabase.createClient(c.url,c.anonKey);}catch(_){this.backend=null;}},
  localAll(){try{const r=localStorage.getItem(this.STORAGE_KEY);if(!r)return this.getDefaults();const x=JSON.parse(r);return Array.isArray(x)?x:this.getDefaults();}catch{return this.getDefaults();}},
  getDefaults(){return[
  ...
  sanitize(t){return typeof t==='string'?t.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g,'').trim().slice(0,this.MAX_LENGTH):'';},
  canSubmit(){return Date.now()-this.lastSubmit>=this.RATE_LIMIT_MS;},
  async getAll(){
    if(this.backend){const {data,error}=await this.backend.from('messages').select('id,content,created_at,author_name,avatar').order('created_at',{ascending:true}).limit(200);
      if(!error&&Array.isArray(data))return data.map(m=>({id:m.id,content:m.content,createdAt:m.created_at,authorName:'شبح',avatar:'👻'}));}
  ...
  async add(content){
    ...
    if(this.backend){const {data,error}=await this.backend.from('messages').insert({content:cleaned,author_name:'شبح',avatar:'👻'}).select('id,content,created_at').single();
      if(error)return{ok:false,error:'تعذر إرسال الرسالة الآن'};this.lastSubmit=Date.now();
      return{ok:true,message:{id:data.id,content:data.content,createdAt:data.created_at,authorName:'شبح',avatar:'👻'}};}
    const ms=this.localAll(), msg={id:'m_'+Date.now()+'_'+Math.random().toString(36).slice(2,7),content:cleaned,createdAt:new Date().toISOString(),authorName:'شبح',avatar:'👻'};
    ms.push(msg);if(ms.length>100)ms.splice(0,ms.length-100);try{localStorage.setItem(this.STORAGE_KEY,JSON.stringify(ms));}catch(_){}this.lastSubmit=Date.now();return{ok:true,message:msg};
  }
};
